    ...
    <div class="flex justify-between mb-8 items-end">
      ...
      <NuxtLink to="/videos/favoritos">
        <Button class="gap-2 relative">
          ...
          <Inbox />
          <span
            v-if="videosFavoritos.length > 0"
            class="text-red-400 absolute right-2 bottom-5 bg-slate-800 h-5 w-5 rounded-full"
          >
    ...
    >
      ...
      >
        ...
        <div class="w-full flex justify-between gap-4 mt-3">
          <Button class="gap-2" @click="addFavorito(video)">
            Favoritos
            <Plus />
          </Button>
...
const { data: videos, error, pending } = await useLazyFetch("/api/v1/videos");
